...
public class GradleModuleHandlerTest {
    ...
    public void publicConstructorTest(){
        GradleModuleHandler gradleModuleHandler = new GradleModuleHandler();

        assertEquals("settings.gradle", gradleModuleHandler.getBuildFileName());
        assertEquals("gradle", gradleModuleHandler.getBuildSystemName());
    }

    @Test
    public void buildProperties() {
        Mockito.when(moduleHandlerMock.configService.get(ConfigService.BQ_VERSION)).thenReturn("2.0");
        ...
        Properties properties = moduleHandlerMock.buildProperties(components, outputRoot, parentFile).build();

        assertNotNull(properties.get("input.path"));
        assertEquals("templates/gradle-module/", properties.get("input.path"));
